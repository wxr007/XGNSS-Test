    ...
    : QObject(NULL)
{
    ...
    m_WebSocketServerThread = new WebSocketServerThread(NULL);
    ...
    connect(m_DataParser, SIGNAL(sgnNMEA(const QByteArray)), m_WebSocketServerThread, SIGNAL(sgnWrite(const QByteArray)), Qt::QueuedConnection);
}
ThreadManager::~ThreadManager()
{
    m_SerialThread->stop();
